# Interactive art tutorial
Welcome to the in depth art tutorial! This tutorial is designed to be run
...
In order to follow along, you must have artifact installed somewhere on your PATH.
Check out the [User Guide](https://github.com/vitiral/artifact/wiki/User-Guide)
for instructions.
...
Just run `art tutorial` and read the created `tutorial.toml` file that
...
> **Run `art tutorial 2` to reset the local directory to this stage**
...
accomplish, but then complete it in pieces. **artifact** will help you track which
part is complete!
...
Try `art ls purpose -p` to search for all items with "purpose" in the name, you
can see that three purpose requirements appear.

> ### Exercise 3:
> Play around with the `art ls` command a little more to get used to it, we will
...
> made. You can always return it to it's original state with `art tutorial 3`
...
### Exercises
 1. run `art ls ARTIFACT` on an artifact that is tagged in source. Now change
...
We can see that `art ls` is not the right tool for the job -- from looking at
...
> fixing errors until there are no errors, then run `art ls` to see if the
> current status makes sense.

--------------------------------------------------
## Documenting your own project
To start documenting your own project, run `art init` in your project and edit
`.art/settings.toml` with the paths on where to find your code-implementations
and documents.
...
Here are a few parting words of advice:
 ...
 2. Keep your artifacts fairly high level -- don't try to design every detail
      using artifact. Using artifact does not mean that you shouldn't use code comments!
...
artifact. I leave it as an exercise for the reader to finish the project in whichever
language you are most comfortable. Have some fun with the artifact tool, try to break
